...
Shared_Settings.Extra_Properties = true;
...
Settings.Solid_Test_Time = 100; % ps. Sets time for combination of solid equilibration + test time
...
% Other options
Shared_Settings.Structures = {'Rocksalt' 'Wurtzite' 'Sphalerite' 'NiAs' 'FiveFive' 'AntiNiAs' 'BetaBeO' 'CsCl'};
...
switch lower(computer)
    case 'unbearabull'
        ...
        for jdx = 1:length(Salts)
            ...
            Settings_array(idx) = Fit_WBK_to_BH(Settings_array(idx),Model_BH);
            Settings_array(idx).GaussianCharge = false;
...
calc_cmd = 'matlab -nodisplay -r "LiX_Minimize_Thermalize(##INPUTFILE##)" >> ##LOGFILE##';
...
for idx = 1:length(Settings_array)
    ...
    Settings.scratch_dir = Settings.WorkDir;
    Settings.OuterDir = Settings.WorkDir;
    Settings.initial_opt_type = true; % required for using systematic folders
