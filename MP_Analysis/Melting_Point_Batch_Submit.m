...
Shared_Settings.N_Calc = 6; % Number of chained calculations
...
switch lower(computer)
    case 'graham'
        ...
        %% Alexandria Model (Gaussian charge + polarization) - emtol 1e-2 + PR(1ps) for LiI
        Salts = {'LiI'};
        Theories = {'BF'};
        vdW_Type = {'WBK'}; % Allowed vdW types: 'WBK', 'BK', 'LJ_12-6', 'LJ_8-6'
        Structures = {'Rocksalt'};
        ...
        for jdx = 1:length(Salts)
            ...
            for kdx = 1:length(Theories)
                ...
                for sdx = 1:numel(Structures)
                    ...
                    Settings_array(idx).Structure = Structure; % Input model(s) to use: JC, JC3P, JC4P, TF, BH
                    Settings_array(idx).Model = 'Alexandria'; % Name of the current model. Leave blank for the default JC/TF/BH model
                    Settings_array(idx).JobID = [Theory '_emtol_1e-2']; % An ID that is tacked onto the folder name of all current jobs
                    Settings_array(idx) = Alexandria_Potential_Parameters(Settings_array(idx),'vdW_Type',vdW_Type{kdx});
                    Settings_array(idx).GaussianCharge = true;
                    Settings_array(idx).Polarization = true;
        ...
        %% Alexandria Model (Gaussian charge + polarization) - emtol 1e-2 + PR(10ps) barostat for LiI
        Salts = {'LiI'};
        Theories = {'BF'};
        vdW_Type = {'WBK'}; % Allowed vdW types: 'WBK', 'BK', 'LJ_12-6', 'LJ_8-6'
        Structures = {'Rocksalt'};
        ...
        for jdx = 1:length(Salts)
            ...
            for kdx = 1:length(Theories)
                ...
                for sdx = 1:numel(Structures)
                    ...
                    Settings_array(idx).Structure = Structure; % Input model(s) to use: JC, JC3P, JC4P, TF, BH
                    Settings_array(idx).Model = 'Alexandria'; % Name of the current model. Leave blank for the default JC/TF/BH model
                    Settings_array(idx).JobID = [Theory '_emtol_1e-2_PR10']; % An ID that is tacked onto the folder name of all current jobs
                    Settings_array(idx) = Alexandria_Potential_Parameters(Settings_array(idx),'vdW_Type',vdW_Type{kdx});
                    Settings_array(idx).GaussianCharge = true;
                    Settings_array(idx).Polarization = true;
        ...
        %% Alexandria Model (Gaussian charge + polarization) - emtol 1e-2 + Berendsen(10ps) barostat for LiI
        Salts = {'LiI'};
        Theories = {'BF'};
        vdW_Type = {'WBK'}; % Allowed vdW types: 'WBK', 'BK', 'LJ_12-6', 'LJ_8-6'
        Structures = {'Rocksalt'};
        ...
        for jdx = 1:length(Salts)
            ...
            for kdx = 1:length(Theories)
                ...
                for sdx = 1:numel(Structures)
                    ...
                    Settings_array(idx).Structure = Structure; % Input model(s) to use: JC, JC3P, JC4P, TF, BH
                    Settings_array(idx).Model = 'Alexandria'; % Name of the current model. Leave blank for the default JC/TF/BH model
                    Settings_array(idx).JobID = [Theory '_emtol_1e-2_Berendsen10']; % An ID that is tacked onto the folder name of all current jobs
                    Settings_array(idx) = Alexandria_Potential_Parameters(Settings_array(idx),'vdW_Type',vdW_Type{kdx});
                    Settings_array(idx).GaussianCharge = true;
                    Settings_array(idx).Polarization = true;
        ...
        for jdx = 1:length(Salts)
            ...
            for kdx = 1:length(Theories)
                ...
                for sdx = 1:numel(Structures)
                    ...
                    Settings_array(idx).Structure = Structure; % Input model(s) to use: JC, JC3P, JC4P, TF, BH
                    Settings_array(idx).Model = 'Alexandria'; % Name of the current model. Leave blank for the default JC/TF/BH model
                    Settings_array(idx).JobID = [Theory '_emtol_1e-2_Berendsen1']; % An ID that is tacked onto the folder name of all current jobs
                    Settings_array(idx) = Alexandria_Potential_Parameters(Settings_array(idx),'vdW_Type',vdW_Type{kdx});
                    Settings_array(idx).GaussianCharge = true;
                    Settings_array(idx).Polarization = true;
        ...
        %% Alexandria Model (Gaussian charge + polarization) - emtol 1e-2 + Berendsen(10ps,lit. compress.) barostat for LiI
        Salts = {'LiI'};
        Theories = {'BF'};
        vdW_Type = {'WBK'}; % Allowed vdW types: 'WBK', 'BK', 'LJ_12-6', 'LJ_8-6'
        Structures = {'Rocksalt'};
        ...
        for jdx = 1:length(Salts)
            ...
            for kdx = 1:length(Theories)
                ...
                for sdx = 1:numel(Structures)
                    ...
                    Settings_array(idx).Structure = Structure; % Input model(s) to use: JC, JC3P, JC4P, TF, BH
                    Settings_array(idx).Model = 'Alexandria'; % Name of the current model. Leave blank for the default JC/TF/BH model
                    Settings_array(idx).JobID = [Theory '_emtol_1e-2_Berendsen10_litcomp']; % An ID that is tacked onto the folder name of all current jobs
                    Settings_array(idx) = Alexandria_Potential_Parameters(Settings_array(idx),'vdW_Type',vdW_Type{kdx});
                    Settings_array(idx).GaussianCharge = true;
                    Settings_array(idx).Polarization = true;
