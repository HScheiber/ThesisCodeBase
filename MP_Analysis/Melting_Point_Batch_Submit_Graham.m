%%%%% Melting_Point_Batch_Submit_Graham %%%%%%
...
Shared_Settings.JobSettings.N_Calc = 3; % Number of chained calculations
...
Shared_Settings.JobSettings.MPI_Ranks = 4; % Sets the number of MPI ranks (distributed memory parallel processors). -1 for auto
...
Shared_Settings.lb = 400; % K, lower bound on MP search
...
%% Set62: 11000 atoms default settings (full debugged 2022-04-29) - reproduciblility test with tmax = 1 ns x 100
...
for kdx = 1:length(Reps)
    ...
    Settings_array(idx).MaxCheckTime = 1000; % ps. Max time for melting/freezing runs
