...
if strcmpi(Settings.RefStructure,'Liquid')
    ...
    % Update the freeze trace to include any unclassified
    Unclassed = (~T_dat.Freeze_Trace & ~T_dat.Melt_Trace);
    
    T_dat.Freeze_Trace = (T_dat.Freeze_Trace | Unclassed);
    T_dat.T_Freeze_Trace = T_dat.T_Trace(T_dat.Freeze_Trace);
    T_dat.dT(1) = max([T_dat.T_Freeze_Trace lb]);
    T_dat.dT(2) = min([T_dat.T_Melt_Trace ub]);
    ...
    if any(T_dat.T_Freeze_Trace > T_Melt_Min) || any(T_dat.T_Melt_Trace < T_Freeze_Max)

        disp('Warning: updating classification of at least one freezing/melting point.')
        disp('This occurs when a new melted T is found to be lower than a previous freezing T')
        disp('or when a new freezing T is found to be higher than a previous melting T.')
        T_dat.Melt_Trace = T_dat.T_Trace >= T_Melt_Min;
        T_dat.Freeze_Trace = T_dat.T_Trace <= T_Freeze_Max;
    ...
    while TBracket > Settings.BracketThreshold
        ...
        % Info
        disp(['Checked temperatures that melted: ' num2str(sort(T_dat.T_Melt_Trace),'%.4f K    ')])
        disp(['Checked temperatures that did not melt:  ' num2str(sort(T_dat.T_Freeze_Trace,'descend'),'%.4f K    ')])
