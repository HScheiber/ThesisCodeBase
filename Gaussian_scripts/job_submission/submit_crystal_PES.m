...
pausetime = 1; % time to pause between in seconds
...
if nCores/nTasks_per_Node > 1
    ...
    gCores = num2str(nTasks_per_Node);
else
    linda='';
	gCores = num2str(nCores);
end
...
for i = 1:length(theory)
    ...
    for j = 1:length(basis)
        ...
        for k=1:length(a)
			...
			newtext = strrep(newtext,'##CORES##',gCores);
