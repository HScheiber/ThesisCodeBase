function [U,C6_out] = BE_Potential_Generator(Settings,varargin)
...
for interaction = {'MX' 'XX' 'MM'}
    ...
    U_Total  =  k_0*(e_c^2).*q.(int(1))*q.(int(2)).*U.(int).f0  + C6_out.(int).*U.(int).g  + U.(int).h; % Full potential
    U_Coul   =  k_0*(e_c^2).*q.(int(1))*q.(int(2)).*U.(int).f0; % Coulomb potential
