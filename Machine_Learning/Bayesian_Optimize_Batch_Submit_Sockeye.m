%%%%% Bayesian_Optimize_Batch_Submit_Sockeye %%%%%%
%% INFO %%
...
clear;
Calculation.N_Calc = 8; % Number of chained calculations
Calculation.hours = 6; % Number of hours for each job in the job chain
Calculation.slurm_memory = '3800M'; % memory per core
Calculation.nodes = 1;
Calculation.cpus_per_node = 8;
...
for idx = 1:length(Models)
    ...
    if check_complete
        ...
        if ~isempty(obs)
            disp([Model.Theory ' Model ' Model.Trial_ID ': Job already completed. Skipping Job Submission.'])
            continue
        end
    end
