...
gammas = [map_logspace(-2,0.004,4,100*Density) ...
          fliplr(map_logspace(-1,5.4,4,30*Density)) ...
          fliplr(map_logspace(-1,5.96,5.405,150*Density)) ...
          map_logspace(-1,6.04,6.3,100*Density) ...
          map_logspace(-1,6.3,7,30*Density) ...
          repmat(7,1,10*Density) ...
          linspace(7,8,50*Density) ...
          fliplr(map_logspace(-1,8.7,8.01,150*Density)) ...
          map_logspace(-2,8.71,100,70*Density)];
...
title_obj = title(ax,['$\gamma_{ij} = 0.0000$' newline '$\varepsilon_{ij} = -1$' newline '$R_{0,ij} = 1$'],...
    'FontSize',fs,'Interpreter','latex','Color',[0 0 0],...
    'Position',[2 1.5 0]);
ylabel(ax,'$u^{\textrm{BH}}_{ij}(r_{ij})$','FontSize',fs,'Interpreter','latex','Color',[0 0 0])
xlabel(ax,'$r_{ij}$','FontSize',fs,'Interpreter','latex','Color',[0 0 0])
...
txt_obj = text(r0,epsilon+diff(ylims)*0.05,'$(R_{0,ij},-\varepsilon_{ij})$','fontsize',fs,'interpreter','latex');
...
%     fprime = fpfun(r);
    ...
    title_obj.String = ['$\gamma_{ij} = ' num2str(gamma,'%.4f') '$' newline ...
        '$\varepsilon_{ij} = ' num2str(epsil,'%+i') '$' newline '$R_{0,ij} = 1$'];
