...
@l1i=(6);
@l1a=(64,128,256,512,1024,2048,4096,8192);
...
foreach $l1 (@l1i) {
 ...
 system "echo Submitting $l1s Molecules at $nCores cores for $hours_calc:$mins_calc ";
 
 # Args[default]: queue[-1] cores cores/node[-1] node-type[-1] mem/core[-1] hours mins name inp out #links exe
 system "submit_cp2k.pl -1 $nTasks $nTasks_per_Node -1 -1 $hours_calc $mins_calc SSc-ALMO-$l1s-$nCores input.$out_id.inp output.$out_id.$sTasks.out 1 $cp2k_ver";
