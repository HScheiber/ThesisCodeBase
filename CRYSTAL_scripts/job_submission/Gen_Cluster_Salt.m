function Coordinates_Out = Gen_Cluster_Salt(Salt,Structure,Coordinates_In,BSSE_id,...
	NStars,Rmax,a,b,c,N_Cluster)
...
[FC_Metal,FC_Halide] = UnitCell_FractionalCoords(FC_Metal,FC_Halide,Structure);
Transform_Mat = GenTransformMatrix(Input);

a_vec = a.*Transform_Mat(1,:);
b_vec = b.*Transform_Mat(2,:);
c_vec = c.*Transform_Mat(3,:);

Transform_Mat = [a_vec; b_vec; c_vec];
% Metal_Cart_Coordinates = nan(NF,3);
% Halide_Cart_Coordinates = nan(NF,3);
% for i = 1:NF
