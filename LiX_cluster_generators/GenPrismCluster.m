function [Final_Coordinates,Final_Identities] = GenPrismCluster(Settings,Geometry)

N_points = 4; % number of sides on the prism (6 = hexagon)
Cutoff = 50; % Angstroms, maximum search distance
Rot = {[0 0 1 deg2rad(45)]}; % Sets the final rotation in xy plane
...
Prism_center = Settings.Cluster_Center*Transform_Matrix;
Ref_axis_major = Settings.lattice_plane*Recip_Transform_Matrix./norm(Settings.lattice_plane*Recip_Transform_Matrix);
Ref_axis_minor = (Settings.Minor_axis*Transform_Matrix./norm(Settings.Minor_axis*Transform_Matrix));%*axang2rotm([Ref_axis_major 2*pi/8]);
Prism_height = norm(( (Ref_axis_major/Transform_Matrix)./norm(Ref_axis_major/Transform_Matrix) )*Transform_Matrix)*Settings.Prism_height;
...
Search_N = Geometry.FC*Transform_Matrix;
Search_a = [0 -Ma:-1 1:Ma]' * a1_vec;
Search_b = [0 -Mb:-1 1:Mb]' * a2_vec;
Search_c = [0 -Mc:-1 1:Mc]' * a3_vec;

Search_Combs = combvec(1:Geometry.N,1:(2*Ma+1),1:(2*Mb+1),1:(2*Mc+1));
...
Sorted_Identities = Geometry.AtomNames(Identities_d_Major_sel(Sort_idx));
...
M_Sel_Coordinates = M_Sorted_Coordinates(1:Settings.N_Cluster/2,:);
X_Sel_Coordinates = X_Sorted_Coordinates(1:Settings.N_Cluster/2,:);

% Recombine back into single cluster
Final_Coordinates = [M_Sel_Coordinates; X_Sel_Coordinates];
Final_Identities = [repmat({Geometry.Metal},Settings.N_Cluster/2,1); repmat({Geometry.Halide},Settings.N_Cluster/2,1)];


if Settings.ShowCluster
    %% Print into xyz format
    filename = fullfile(pwd,[Settings.Salt '_' Settings.Structure '_N' num2str(Settings.N_Cluster) '_Cylinder.xyz']);
    fid = fopen(filename, 'a');
    % First two lines
    fprintf(fid,[num2str(Settings.N_Cluster) newline ...
        Settings.Salt ' ' Settings.Structure ' Cluster of size ' num2str(Settings.N_Cluster) newline]);

    for jdx = 1:Settings.N_Cluster
