Salt = 'wustite';
Structure = '110';
cif_filename = 'wustite.cif';
N_Cluster = 2376; % Number of atoms in each slab. must be an even number to ensure charge neutrality
lattice_plane = [1 1 1]; % The lattice plane to cut along
...
Cluster_Center = [1/3 1/3 1/3]; % Location of cluster center in units of a, b, and c vectors
Prism_height = 2; % Maximum cylinder height in units of the chosen axis
Cutoff = 50; % Angstroms, maximum search distance
second_slab = true; % add in a second mirrored slab?
slab_distance = 45; % Slab separation in Angstrom

autorotate_z = true; % Automatically aligns the slab along the z-axis when true
...
try
    ...
    Identities_UC = cif_dat(strcmp({cif_dat.name},'atom_site_label')).val; % usually these are elements
end
%Labels_UC = cif_dat(strcmp({cif_dat.name},'atom_site_label')).val; % atom labels


%% Apply symmetry operations
symm_ops = cif_dat(strcmp({cif_dat.name},'space_group_symop_operation_xyz')).val;

for idx = 1:length(symm_ops)
    symm_op = symm_ops{idx};
    
    ops = regexp(symm_op,'(.+?),(.+?),(.+)','tokens','once');
    
    for jdx = 1:N
        current_label = Identities_UC{jdx};
        
        x = Frac_Coordinates(jdx,1);
        y = Frac_Coordinates(jdx,2);
        z = Frac_Coordinates(jdx,3);
        
        ops_x = strrep( strrep( strrep( ops{1},'x',num2str(x,'%.15f') ),'y',num2str(y,'%.15f') ),'z',num2str(z,'%.15f'));
        ops_y = strrep( strrep( strrep( ops{2},'x',num2str(x,'%.15f') ),'y',num2str(y,'%.15f') ),'z',num2str(z,'%.15f'));
        ops_z = strrep( strrep( strrep( ops{3},'x',num2str(x,'%.15f') ),'y',num2str(y,'%.15f') ),'z',num2str(z,'%.15f'));
        
        x_new = mod(eval(ops_x),1);
        y_new = mod(eval(ops_y),1);
        z_new = mod(eval(ops_z),1);
        xyz_new = [x_new y_new z_new];
        
        % Check if already in the fractional coordinates
        if ~any(ismember(Frac_Coordinates,xyz_new,'rows'))
            Frac_Coordinates(end+1,:) = xyz_new;
            Identities_UC{end+1} = current_label;
        end
    end
end
N = size(Frac_Coordinates,1);
...
if autorotate_z
    ...
    a1_vec = Crystal.Transform(1,:);
    a2_vec = Crystal.Transform(2,:);
    a3_vec = Crystal.Transform(3,:);
    ...
    z_angle = vec_angle_signed(a3_vec,Ref_axis_major,V_normal);
...
% Prepare to print
if second_slab
    filename = fullfile(pwd,[Salt '_' Structure '_N' num2str(N_Tot) '_MirroredSlabs.xyz']);
else
    filename = fullfile(pwd,[Salt '_' Structure '_N' num2str(N_Tot) '_Slab.xyz']);
end
