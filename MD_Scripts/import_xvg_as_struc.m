...
function DataStruc = import_xvg_as_struc(filename)
...
xaxislabel_row = ~cellfun('isempty', xaxis);
...
yaxislabel_row = ~cellfun('isempty', yaxis);
...
for i=1:nColumns-1
    ...
    yaxis_legend_row = ~cellfun('isempty', yaxis);
...
DataStruc.Time = Data(:,1);

for idx = 1:length(yaxis_all_legends)
    ylab = regexprep(yaxis_all_legends{idx},{' ' 'En\.'},{'_' 'Energy'});
    DataStruc.(ylab) = Data(:,idx+1);
end
