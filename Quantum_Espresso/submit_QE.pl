...
$account="rrg-patey-ad"; # charging account
@exe = (
 "pw.x",
 "/cvmfs/soft.computecanada.ca/easybuild/software/2017/avx2/MPI/intel2016.4/openmpi2.1/quantumespresso/6.1/qe-6.1/ThesisCodeBase/pw.x"
);

$args=11;
if ($#ARGV!=$args) {
 print "Versions of executable. The first option - QuantumEspresso in your path:\n";
 for ($ii=0; $ii<=$#exe; $ii++) {
  printf "%3d - %-100s\n", $ii, $exe[$ii];
 }
...
VERBATIM
  ...
  #Copy input file to temp file, needed for quantum espresso for some reason
  system("cp $name_in input_tmp.in");
