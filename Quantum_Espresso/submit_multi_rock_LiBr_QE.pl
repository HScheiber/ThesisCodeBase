...
foreach $l1 (@l1i) {
 ...
 system "sed -e \'s/##$l1l##/$l1s/g\' $workDir/LiBr_rock.template > input.$l1s.inp";

 # Args[default]: queue[-1] cores cores/node[-1] node-type[-1] mem/core[-1] hours mins name inp out #links exe
 system "submit_QE.pl -1 $nTasks $nTasks_per_Node -1 -1 $hours_calc $mins_calc LiBr-R-$l1s-$nCores input.$l1s.inp output.$l1s.out 1 $QE_ver";
