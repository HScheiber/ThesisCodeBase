...
for interaction = {'MX' 'XX' 'MM'}
    ...
    elseif Settings.C6_Damp.(int) == 2
        ...
        f6.(int) = f_r.(int).*(1 - f6sum.*exp(-alpha.*r));
        ...
        df6.(int) = f_r.(int).*((alpha.*exp(-alpha.*r)).*f6sum ...
            - (exp(-alpha.*r)).*df6sum) + df_r.(int).*(1 - f6sum.*exp(-alpha.*r));
    ...
    elseif Settings.C6_Damp.N.(int) == 2
        ...
        fNsum = 0;
        for k = 0:NV
            fNsum = fNsum + ((alpha.*r).^k)./factorial(k); 
        end
        fN.(int) = f_r.(int).*(1 - fNsum.*exp(-alpha.*r));

        % Calculate C6 damping derivatives
        dfNsum = 0;
        for k = 1:NV
            dfNsum = dfNsum + k.*(alpha.^k).*(r.^(k-1))./factorial(k);
        end
        dfN.(int) = f_r.(int).*((alpha.*exp(-alpha.*r)).*fNsum ...
            - (exp(-alpha.*r)).*dfNsum) + df_r.(int).*(1 - fNsum.*exp(-alpha.*r));
    ...
    if Settings.S.N.(int).Value > 0
        ...
        U_add  = - fN.(int).*NS./(r.^NV);
        dU_add = + fN.(int).*NS.*NV./(r.^(NV+1)) ...
                 - dfN.(int).*NS./(r.^NV);
